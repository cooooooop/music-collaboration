...
public class MixPage extends Composite implements IUpdatable, IDAOEditor {
	...
	@UiTemplate("uibinder/MixPage.ui.xml")
	interface MyUiBinder extends UiBinder<Widget, MixPage> { }
	...
	public MixPage() {
		...
		userNameLabel.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				for(NavigationEventHandler handler : handlers) {
					handler.onMemberPageNavigation(new NavigationEvent(MixPage.this.mixDAO.getOwnerUserDAO()));
		        }
			}
		});
	...
	public void setMixDAO(MixDAO mixDAO) {
		...
		this.userNameLabel.setText(mixDAO.getOwnerUserDAO().getNickname());
		this.titleLabel.setText(mixDAO.getMixName());
		
		this.richTextArea.setHTML(mixDAO.getUserContent());
		
		saveButton.addClickHandler(new ClickHandler() {
			
			@Override
			public void onClick(ClickEvent event) {
				MixPage.this.mixDAO.setUserContent(richTextArea.getHTML());
				for(DAOEventHandler handler : daoHandlers) {
					handler.onSaveMix(new DAOEvent(MixPage.this.mixDAO, MixPage.this));
		        }
			}
		});
	}
